defmodule AOC.Day11b do
  ...
  defmodule OctoServer do
    ...
    def handle_call(:empty_energy?, _caller, grid) do
      empty =
        grid
        |> Enum.map(fn {_id, %{value: energy}} -> energy end)
  ...
  def solution(path) do
    ...
    {:ok, pid} = GenServer.start_link(AOC.Day11b.OctoServer, grid)
    step(pid, 1)
  end
  ...
  def step(pid, n) do
    ...
    if GenServer.call(pid, :empty_energy?) do
      n
    else
      step(pid, n + 1)
    end
