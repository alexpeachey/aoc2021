defmodule AOC.Day16b do
  ...
  def evaluate({packet, _}), do: evaluate(packet)
  def evaluate(%{type: 4, payload: value}), do: value

  def evaluate(%{type: 0, payload: packets}) do
    packets
    |> Enum.map(&evaluate/1)
    |> Enum.sum()
  end

  def evaluate(%{type: 1, payload: packets}) do
    packets
    |> Enum.map(&evaluate/1)
    |> Enum.product()
  end

  def evaluate(%{type: 2, payload: packets}) do
    packets
    |> Enum.map(&evaluate/1)
    |> Enum.min()
  end

  def evaluate(%{type: 3, payload: packets}) do
    packets
    |> Enum.map(&evaluate/1)
    |> Enum.max()
  end

  def evaluate(%{type: 5, payload: packets}) do
    [val1, val2] =
      packets
      |> Enum.map(&evaluate/1)

    if val1 > val2, do: 1, else: 0
  end

  def evaluate(%{type: 6, payload: packets}) do
    [val1, val2] =
      packets
      |> Enum.map(&evaluate/1)
  ...
  def evaluate(%{type: 7, payload: packets}) do
    [val1, val2] =
      packets
      |> Enum.map(&evaluate/1)
